...
for step in range(11):
    ...
    graph_dd_res = []
    for analysisType in AnalysisType:
            with open(f'../../Graph-Reach_test/result/{step*1000}/{analysisType}{0}.res', 'r') as f2:
                 for line in f2:
                     if analysisType == "AliasAnalysis":
                         if ("Reachability" in line) and (not ("not" in line)) :
                            graph_alias_res.append(0)
                         if ("not Reachability" in line) :
                            graph_alias_res.append(1)
                     if analysisType == "DataDepAnalysis":
                         if ("Reachability" in line) and (not ("not" in line)) :
                            graph_dd_res.append(0)
                         if ("not Reachability" in line):
                            graph_dd_res.append(1)
    ...
    i = 0
    flag = 0
    for x in graph_alias_res:
        if ( x != yices_alias_res[i] or x!=z3_alias_res[i] or x!=cvc5_alias_res[i] or x!=platsmt_alias_res[i] ):
            flag = 1
        i = i + 1

    j = 0
    for x in graph_dd_res:
        if ( x != yices_dd_res[j] or x!=z3_dd_res[j] or x!=cvc5_dd_res[j] or x!=platsmt_dd_res[j] ):
            flag = 1
        j = j + 1
